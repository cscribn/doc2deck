...
2. Install and authenticate Gemini CLI: `npm install -g @google/gemini-cli`, then run `gemini` once to sign in.
3. Place `template.pptx` and `source.docx` at project root (or override paths in `.env`).
4. Run from project root: `./gradlew run` → `final_presentation.pptx` (step progress on stdout; errors only in logs).
...
**Template:** `${variableName}` placeholders in single uninterrupted text runs; keys in [prompts/prompt_presentation_keys.md](prompts/prompt_presentation_keys.md).
